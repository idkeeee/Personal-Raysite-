...
function ensureScaffold() {
  ...
  // FAB
  if (!$('.fab-add')) {
    const fab = el('button', { className: 'fab-add', title: 'Add card', innerHTML: '+' });
  ...
  if (!$('.sheet')) {
    const sheet = el('div', { className: 'sheet', role: 'dialog', 'aria-modal': 'true', 'aria-hidden': 'true' }, [
      ...
      el('div', { className: 'sheet-header' }, [
        el('h3', { className: 'sheet-title', textContent: 'Card' }),
...
async function upsertCard(card) {
  ...
  const { data, error } = await supabase.from('wakeup_cards').upsert(payload).select().single();
  if (error) throw error;
  return data;
}

async function deleteCard(id) {
  const { error } = await supabase.from('wakeup_cards').delete().eq('id', id);
  if (error) throw error;
}
...
function subscribeRealtime() {
  try {
    supabase
      .channel('wakeup_cards_changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'wakeup_cards' }, payload => {
    ...
    console.warn('realtime subscribe failed (ok to ignore on static hosting)', e);
  }
}
